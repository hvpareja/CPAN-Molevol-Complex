...
contact interfaces within a protein complex. The method C<Coevolution::Compcoe-E<gt>run>
launchs the whole analisys. Also, such workflow is divided into the following steps:

=over 4

=item B<Step 1>, Structural analisys: C<Coevolution::Compcoe-E<gt>run_struct>

=item B<Step 2>, Sub-sets filtering: C<Coevolution::Compcoe-E<gt>run_filtering>

=item B<Step 3>, Sub-alignments building: C<Coevolution::Compcoe-E<gt>run_subalign>

=item B<Step 4>, Evolutionary calcs: C<Coevolution::Compcoe-E<gt>run_yang>

=item B<Step 5>, Statistical analisys: C<Coevolution::Compcoe-E<gt>run_stats1>
...
The new class method construct a new L<Coevolution::Compcoe> object. The returned
object can be used to perform several evolutive analisys. C<new> accepts the
...
A valid DNA alignment file path. The alignment must correspond with the specified
...
An error margin for sth. For instance: if is set to 0.01, residues with exposure
...
Once each analisys has been performed, the resulting data is stored in other
setable attributes:
...
[Array] A table with the structural information calculated by Coevolution::Contact.pm
and DSSP
...
[Hash] Results for evolutive analisys. Each item contains the results for a
...
set_attribute, respectively. For example, to get the value for the proximity threshold:
...
Please report any bugs or feature requests to C<bug-coevolution-compcoe at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Coevolution-Compcoe>.
